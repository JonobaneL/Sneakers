...
const PasswordReset = () => {
    ...
    const [toast,setToast] = useState({type:'success',title:'',triger:false})
    const handleSubmit = async(e) =>{
        e.preventDefault();
        try{
            setIsLoading(true)
            await resetPassword(email.value)
            setToast({type:'none',title:'Check your inbox for instructions',triger:true})
        }catch{
            setToast({type:'error',title:'Failed to reset password',triger:true})
        } 
        setIsLoading(false)
        ...
        <div className={styles['password-reset']}>
            <div className={styles.content}>
                ...
                <div className={styles.message}>
                    <p>Back to <Link to='/log-in'>Log In</Link></p>

                </div>
            </div>
        <Toast type={toast.type} title={toast.title} triger={toast.triger} closeHandler={value=>setToast(p=>{ return{...p,triger:value}})}/>
        </div>
    );
};
