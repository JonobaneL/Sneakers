...
const UserInfoSettings = () => {
  ...
  const phoneNumber = useInput(
    userInfo.phone || "",
    { minLength: 12 },
    { minLength: "Please enter your number" }
  );
  ...
  const confirmPassword = useInput(
    "",
    { minLength: 6, isMatch: password.value },
    {
      minLength: "Password must has at least 6 characters",
  ...
  const applyHandler = () => {
    ...
    if (phoneNumber.value !== formatPhoneNumber(userInfo.phone)) {
      promises.push(
        updateUserPhone({ uid: currentUser.uid, phone: phoneNumber.value })
      );
    }
    if (city !== userInfo.city) {
      promises.push(
        updateUserCity({ uid: currentUser.uid, city: locationResponse[0] })
      );
    }
    ...
    Promise.all(promises)
      .then(() => navigate("/user-profile/info"))
  ...
  return (
    <div className={styles["info-settings"]}>
      <ul className={styles.info}>
        ...
        <li className={styles.info__item}>
          <div className={styles["field-name"]}>Phone:</div>
          <div className={styles.input}>
            <CInput
              height={`${windowSize ? "40" : "50"}`}
              mode="fullBorder"
              ...
              valid={
                phoneNumber.isDurty && phoneNumber.currentErrors.length > 0
              }
            />
          </div>
        </li>
        <li className={styles.info__item}>
          <div className={styles["field-name"]}>City:</div>
          <div className={styles.input}>
            <Autocomplete
              data={locationResponse}
              query={city}
              setQuery={(value) => setCity(value.name)}
            >
              <CInput
                mode="fullBorder"
                height={`${windowSize ? "40" : "50"}`}
        ...
        <li className={styles.info__item}>
          <div className={styles["field-name"]}>Password:</div>
          <div className={styles.input}>
            <ValidationErrorMessages
              durty={password.isDurty}
              errorMessages={password.currentErrors}
            >
              <CInput
                mode="fullBorder"
        ...
        <li className={styles.info__item}>
          <div className={styles["field-name"]}>Confirm Password:</div>
          <div className={styles.input}>
            <ValidationErrorMessages
              durty={confirmPassword.isDurty}
              errorMessages={confirmPassword.currentErrors}
            >
              <CInput
                mode="fullBorder"
                ...
                height={`${windowSize ? "40" : "50"}`}
                valid={
                  confirmPassword.isDurty &&
                  confirmPassword.currentErrors.length > 0
                }
              />
            </ValidationErrorMessages>
          </div>
      ...
      <div className={styles.warning}>Don't change fields to keep the same</div>
