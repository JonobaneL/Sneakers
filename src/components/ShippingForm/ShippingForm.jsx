  ...
  return (
    <div className={styles.shipping}>
      <div className={styles.city} onClick={() => setIsCityModalOpen(true)}>
        <img
          className={styles.city__pointIcon}
          src={mapPoint}
          alt="Map point"
        />
        <p className={styles.city__name}>
          {city.id ? (
            <>
              Your city is
              <span className={styles.city__current}>{city.name}</span>
            </>
          ) : (
            "Choose your city"
          )}
      ...
      <RadioList
        list={[
          ...
          {
            id: "opt4",
            label: "Courier Nova Poshta",
            value: "Nova Poshta2",
            disabled: validRadio,
          },
        ]}
        groupName="shipping"
        callback={(value) => dispatch(setOrderCompany(value.slice(0, -1)))}
      >
        <div className={styles["option-wrapper"]}>
          <Select
            placeholder="Select a post office"
            params={postOffices.map((item) => {
              return { id: item.id, value: `${item.name}, ${item.street}` };
            })}
            getData={handler}
            type="borderType"
            height="45px"
