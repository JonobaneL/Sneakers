  ...
  return (
    <li
      className={`${styles["list-item"]} ${
        checkedItems.includes(item.name) ? styles.active : ""
      }`}
      onClick={() => event(item.name)}
    >
      <div className={styles["checkbox-indicator"]} style={checkboxStyles}>
        <img
          className={styles["checkbox-mark"]}
          src={iconColor == "white" ? whiteMark : darkMark}
          alt="checkbox-indicator"
        />
      </div>
      <p className={styles["list-title"]}>{item.name}</p>
    </li>
  );
};
      ...
      {data.length > 0 ? (
        data.map((item, index) => (
          <CheckBoxItem
            ...
            event={handleClick}
            color={colored ? item.color : "#ffffff"}
          />
        ))
      ) : (
        <p>Not Found</p>
