...
const DropDownList = memo(({ handler, data, seleted }) => {
  const searchCategory = () => {
    if (seleted.length > 0) {
      ...
      const subCategory = mainCategory?.children?.find((item) =>
        seleted.includes(item?.name)
      );
  ...
  return (
    <div className={styles.dropDownList}>
      <div
        style={choosedCategory?.name.length ? VISIBLE : INVISIBLE}
        className={styles["choosed-category"]}
      >
        <p className={styles["all-category"]} onClick={clearEvent}>
          &#60; All categories
        </p>
        <DropDownListItem
          fixed={true}
