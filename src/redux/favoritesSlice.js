...
import {
  getFavorites,
  updateFavorites,
} from "../firebase/favoritesFirebaseAPI";
...
export const fetchFavorites = createAsyncThunk(
  ...
  async (userID) => {
    ...
    response.forEach(
      (item) => (favorites = { ...item.data(), listID: item.id })
    );
  ...
  async (product, { dispatch, getState }) => {
    ...
    try {
      await updateFavorites({
        favoritesID: state.favoritesReducer.listID,
...
const { addToFavoritesAction, removeFromFavoritesAction } =
