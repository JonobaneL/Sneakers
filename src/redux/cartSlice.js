  ...
  async (product, { dispatch, rejectWithValue, getState }) => {
    ...
    try {
      await updateCart({
        ...
        cart: state.cartReducer.shoppingCart,
      });
    } catch (err) {
      console.log(err);
      rejectWithValue();
...
const cartSlice = createSlice({
  ...
  reducers: {
        ...
        state.shoppingCart = state.shoppingCart.map((item) => {
          if (
            ...
            return {
              ...
              cost: item.discountPrice * (item.quantity + 1),
            };
          } else {
            return item;
          }
    ...
    decreaseCartQuantityAction(state, action) {
      const product = { ...action.payload };
      const productResponse = state.shoppingCart.find(
        (item) =>
          item.productID === product.productID &&
          item.modelID === product.modelID &&
          item.size === product.size
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addMatcher(isAnyOf, (state) => {
        state.cartQuantity = state.shoppingCart.reduce(
          (quantity, item) => item.quantity + quantity,
          0
        );
        state.cartSubTotal = state.shoppingCart.reduce(
          (subTotal, item) => item.cost + subTotal,
          0
        );
        const temporaryTotal = state.shoppingCart.reduce(
