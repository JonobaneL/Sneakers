...
export const checkCheckout = (userInfo)=>{
    ...
    const shippingCheck = shippingOptions.every(item=>shippingKeys.includes(item)) || shippingKeys.includes('postOffice')
    const paymentCheck = checkout.paymentMethod.length>0
    const cardCheck = checkout?.card?.cardNumber.length===19 && checkout?.card?.date?.length===5 && checkout?.card?.cvv?.length===3
    ...
    if(checkout?.paymentMethod ==='Credit or Debit Card' && cardCheck===false){
