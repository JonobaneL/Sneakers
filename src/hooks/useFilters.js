...
export const useFiltered = (
  ...
  const [isLoading, productsError, productsResponse] = useAllProducts(
    type,
    male
  );
    ...
    if (category.length > 0) {
      return sortRes.filter((item) => {
        if (category.includes(item.category) && category.length == 1) {
          ...
          category.includes(item["sub_category"])
        ) {
          return item;
        }
      });
        ...
        case "Up to 30%":
          result.push(
            ...filteredColor.filter(
              (item) => item.discount > 0 && item.discount < 30
            )
          );
          break;
        case "30% to 50%":
          result.push(
            ...filteredColor.filter(
              (item) => item.discount >= 30 && item.discount < 50
            )
          );
          break;
        case "50%+":
          result.push(
            ...filteredColor.filter(
              (item) => item.discount >= 50 && item.discount <= 100
            )
          );
          break;
      }
  ...
  const fitleredSize = useMemo(() => {
    if (size.length > 0) {
      return filteredPercent.filter((item) => {
        if (size.some((size_item) => item.sizes.includes(size_item)))
          return item;
      });
...
export const useToShow = (data, limit, current = 1) => {
  const response = useMemo(() => {
    return data.filter(
      (_, index) => index >= current * limit - limit && index < current * limit
    );
  }, [data, current]);
  return response;
};
